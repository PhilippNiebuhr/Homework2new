...
gss_snap_insta_ci_plot <- gss_snap_insta_ci %>%
  ...
  geom_text(aes(label = signif(proportion, digits = 3), vjust = -0.5,  hjust = 6)) + #adds the proportion on top of the bar graph, don't know how to format this as percentages?
  labs(title = "Men and Women say NO to Instagram and Snapchat*", #is this title too strong? 
       subtitle = "*Well at least a large proportion of the ones who took this survey do", #thought he might find this funny lol 
       x = "",
...
cbp1 <- c("2017"="palevioletred","2018" ="seagreen","2019"= "deepskyblue","2020"= "violet")
...
#So below is what Philipp and I did. We were assigned this task, but seems like someone else did it as well. Up to you which one to keep. I think what might be nice would be to incorporate the scale_x_continuous and scale_y_continuous from the above into Philipp and I's graph and then keep that? (btw shortcut to put/remove hash in front of everything is command + shift + c)
...
#   scale_x_continuous(expand = c(0,0),label=scales::comma,breaks = scales::pretty_breaks(n = 4)) +
#   scale_y_continuous(limits = c(-22, 7.5),label=scales::comma,breaks = scales::pretty_breaks(n = 10)) 
